    ...
    public void run() {
        try (socket;
            ...
            while (!socket.isClosed()) {
                ...
                if (!request.action.equals("LOGIN") && !request.action.equals("REGISTER")) {
                    if (authenticatedUserId == null) {
                        out.writeObject(Response.fail("Please sign in first."));
                        out.flush();
                        continue;
                    }
                ...
                if (request.action.equals("LOGIN") && response.success && response.data instanceof java.util.Map<?, ?> user) {
                    Object id = user.get("id");
                    if (id instanceof Number n) authenticatedUserId = n.intValue();
                }
